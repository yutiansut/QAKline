...
export class IndicatorDataProvider extends DataProvider {
  ...
  updateData () {
    ...
    switch (mode) {
      case dataSources.DataSource.UpdateMode.Refresh: {
        this.refresh()
        break
      }
      case dataSources.DataSource.UpdateMode.Append: {
        indic.reserve(ds.getAppendedCount())
        break
      }
      case dataSources.DataSource.UpdateMode.Update: {
        let i
        let last = ds.getDataCount()
  ...
  getMinMaxAt (index, minmax) {
    ...
    let valid = false
    let i
    let cnt = this._indicator.getOutputCount()
    for (i = 0; i < cnt; i++) {

...
export class ChartManager {
  ...
  setChartStyle (dsName, style) {
    ...
    switch (style) {
      ...
      case 'OHLC':
        ...
        if (dp === undefined || !Util.isInstance(dp, dataProviders.MainDataProvider)) {
          dp = new dataProviders.MainDataProvider(dpName)
        ...
        break
      case 'Line':
        dp = new dataProviders.IndicatorDataProvider(dpName)
  ...
  setCurrentDataSource (dsName, dsAlias) {
    ...
    if (cached !== undefined && cached !== null) {
      ...
      cached = new dataSources.MainDataSource(dsAlias)
      this.setDataSource(dsName, cached, true)
  ...
  updateData (dsName, data) {
    ...
    if (data !== undefined && data !== null) {
      ...
      if (ds.getUpdateMode() === dataSources.DataSource.UpdateMode.DoNothing) {
        return true
      }
    } else {
      ds.setUpdateMode(dataSources.DataSource.UpdateMode.Refresh)
    }
  ...
  onToolMouseMove (frameName, x, y) {
    ...
    for (let n in this._dataSources) {
      if (n.indexOf(frameName) === 0) {
        ...
        if (Util.isInstance(ds, dataSources.MainDataSource)) {
  ...
  setMainIndicator (dsName, indicName) {
    ...
    if (indicDp === undefined) {
      indicDp = new dataProviders.IndicatorDataProvider(indicDpName)
  ...
  setIndicator (areaName, indicName) {
    ...
    if (dp === null || dp === undefined || !Util.isInstance(dp, dataProviders.IndicatorDataProvider)) {
      return false
    }
    ...
    if (Util.isInstance(indic, indicators.VOLUMEIndicator)) {
      ...
      let dp = new dataProviders.MainDataProvider(areaName + '.main')
      this.setDataProvider(dp.getName(), dp)
      dp.updateData()
  ...
  removeMainIndicator (dsName) {
    ...
    let indicDpName = areaName + '.secondary'
    let indicDp = this.getDataProvider(indicDpName)
    if (indicDp === undefined || !Util.isInstance(indicDp, dataProviders.IndicatorDataProvider)) {
      return
    }
  ...
  removeIndicator (areaName) {
    ...
    let dp = this.getDataProvider(areaName + '.secondary')
    if (dp === undefined || !Util.isInstance(dp, dataProviders.IndicatorDataProvider)) {
      return
    }
  ...
  getIndicatorParameters (indicName) {
    ...
    let params = []
    let i
    let cnt = indic.getParameterCount()
    for (i = 0; i < cnt; i++) {
  ...
  setIndicatorParameters (indicName, params) {
    ...
    for (n in this._dataProviders) {
      ...
      if (Util.isInstance(dp, dataProviders.IndicatorDataProvider) === false) {
        continue
      }

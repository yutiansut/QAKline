...
export class CToolObject extends NamedObject {
  ...
  acceptMouseDownMoveEvent (x, y) {
    ...
    if (this.state === CToolObject.state.Draw) {
      ...
      let _oldX = pObj.pTimeline.toIndex(this.oldx)
      let _oldY = pObj.pRange.toValue(this.oldy)
      let _newX = pObj.pTimeline.toIndex(x)
      let _newY = pObj.pRange.toValue(y)
      this.oldx = x
      this.oldy = y
      let _difX = _newX - _oldX
      let _difY = _newY - _oldY
      for (let index in this.points) {
        this.points[index].pos.index += _difX
        this.points[index].pos.value += _difY
      }
    }
    return true
  }
...
export class CBandLineObject extends CBiToolObject {
  ...
  isSelected (x, y) {
    ...
    for (let i = 0; i < fibSequence.length; i++) {
      let stageY = sy + (100 - fibSequence[i]) / 100 * (ey - sy)
      if (stageY < y + 4 && stageY > y - 4) {
        this.select()
        return true
      }
    }
